...
$context = strpos(current_url(), '/whatsapp/bots/') !== false ? 'bot' : 'campaign';
?>
...
<?php if (!empty($template)) { ?>
    ...
    $allowd_extension = whatsapp_get_allowed_extension(); 
    ?>
    <?php if (!empty($template['header_data_format']) && 'IMAGE' === $template['header_data_format']) { ?>
        ...
        <div class="view_<?= $context ?>_image <?= (isset($$context) && empty($$context['filename'])) ? 'hide' : '' ?>">
            <?php if (isset($$context)) : ?>
                <div class="row">
                    <div class="col-md-9">
                        <input type="hidden" id="image_url" value="<?= (!empty($$context['filename'])) ? base_url(get_upload_path_by_type($$context['is_bot'] == '1' ? 'template' : $context) . $$context['filename']) : ''; ?>">
                        <img src="<?= base_url(get_upload_path_by_type($$context['is_bot'] == '1' ? 'template' : $context) . $$context['filename']); ?>" class="img img-responsive" height="70%" width="70%">
                    </div>
                    <div class="col-md-3 text-right">
                        <a href="<?= admin_url(WHATSAPP_MODULE . '/' . $context . '/delete_' . $context . '_files/' . $$context['id']) ?>"><i class="fa fa-remove text-danger"></i></a>
                    </div>
                </div>
            <?php endif ?>
        </div>
        <div class="<?= $context ?>_image <?= (isset($$context) && !empty($$context['filename'])) ? 'hide' : '' ?>">
