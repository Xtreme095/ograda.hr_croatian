...
function add_new_sales_item_post($item, $rel_id, $rel_type)
{
    ...
    $it = $CI->db->get(db_prefix() . 'items')->row();
    if(isset($it->commodity_code) && !empty($it->commodity_code)){
        $razmak = $it->commodity_code;
    } else {
        $razmak = 2000;
    }

    $fields = '';
    $lnd = isset($item['length']) ? $item['length'] : 0;
    if($lnd < $razmak) {
        $fields = $lnd; 
    } else {
        while($lnd > $razmak) {
    ...
    $CI->db->insert(db_prefix() . 'itemable', [
                    ...
                    'spacing'          => isset($item['spacing']) ? $item['spacing'] : 0,
