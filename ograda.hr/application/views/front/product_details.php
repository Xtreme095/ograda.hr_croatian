...
function updateProductPrice(quantity) {
    ...
    if (priceDisplay && totalPrice > 0) {
        if (quantity > 1) {
            priceDisplay.textContent = `${basePrice.toFixed(2)}€/ (${totalPrice.toFixed(2)}€ ukupno)`;
        } else {
            priceDisplay.textContent = `${basePrice.toFixed(2)}€/m1 (${totalPrice.toFixed(2)}€ ukupno)`;
        }
    }
}
    ...
    window.updatePrice = function() {
        ...
        if (!isNaN(total) && total > 0) {
            $('.product-material-height-price').text(
                quantity > 1 ? 
                `${finalPrice.toFixed(2)}€/m1 (${total.toFixed(2)}€ ukupno)` : 
                `${finalPrice.toFixed(2)}€/m1 (${total.toFixed(2)}€ ukupno)`
            );
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updatePrice() {
        if (variants && (variants.heights || variants.materials || variants.glass)) {
            ...
            if (priceDisplay) {
                if (quantity > 1) {
                    priceDisplay.textContent = totalPrice.toFixed(2) + "€/m1 (" + finalTotal.toFixed(2) + "€ ukupno)";
                } else {
                    priceDisplay.textContent = totalPrice.toFixed(2) + "€/m1 (" + finalTotal.toFixed(2) + "€ ukupno)";
                }
            }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updatePrice() {
        ...
        if (selectedMaterial && selectedHeight) {
            ...
            if (quantity > 1) {
                priceDisplay.textContent = `${basePrice.toFixed(2)}€/m1 (${total.toFixed(2)}€ ukupno)`;
            } else {
                priceDisplay.textContent = `${basePrice.toFixed(2)}€/m1 (${total.toFixed(2)}€ ukupno)`;
            }
        }
    ...
    if (priceDisplay) {
        if (!isNaN(total) && total > 0) {
            priceDisplay.textContent = quantity > 1 ? 
                `${basePrice.toFixed(2)}€/m1 (${total.toFixed(2)}€ ukupno)` : 
                `${basePrice.toFixed(2)}€/m1 (${total.toFixed(2)}€ ukupno)`;
        } else {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    submitOrderBtn.addEventListener('click', function() {
        ...
        if (priceElement) {
            ...
            // Handle both single price and price with total format (e.g. "100.00€" or "100.00€/m1 (500.00€ ukupno)")
...
<!-- Fixed Product Gallery Styles -->
<style>
/* Improved Product Gallery with Vertical Layout */
.product-single-gallery {
    position: relative;
    margin-bottom: 30px;
}

/* Container for both the thumbnails and main images */
.sidebar-wrapper {
    display: flex;
    flex-direction: row;
}
...
.prod-thumbnail::-webkit-scrollbar-thumb {
...
/* Remove any carousel-related transforms that might interfere */
.product-single-carousel.owl-carousel {
    display: block !important;
}
...
<!-- Fixed Gallery Script -->
<script>
document.addEventListener('DOMContentLoaded', function() {
    // Unified product gallery functionality
    function initProductGallery() {
        // Flag to prevent multiple initializations
        if (window.productGalleryInitialized) return;
        window.productGalleryInitialized = true;
        
        console.log('Initializing vertical product gallery');
        ...
        if (!thumbnails.length || !mainItems.length) {
            console.warn('Gallery elements not found');
            return;
        }
        
        console.log(`Found ${thumbnails.length} thumbnails and ${mainItems.length} main images`);
        ...
        function updateActiveImage(index) {
            ...
            if (thumbnails[index]) {
                ...
                if (thumbnailContainer) {
                    const isVertical = window.innerWidth > 991;
                    
                    if (isVertical) {
                        // Vertical scrolling for desktop
                        const thumbTop = thumbnails[index].offsetTop;
                        const containerHeight = thumbnailContainer.clientHeight;
                        const thumbHeight = thumbnails[index].clientHeight;
                        ...
                        const thumbWidth = thumbnails[index].clientWidth;
            ...
            mainItems.forEach(item => item.style.display = 'none');
            if (mainItems[index]) mainItems[index].style.display = 'block';
        }
        ...
        // Clean up any OwlCarousel initialization that might be interfering
        if (typeof jQuery !== 'undefined' && jQuery.fn.owlCarousel) {
            const $carousel = jQuery('.product-single-carousel');
            if ($carousel.data('owl.carousel')) {
                try {
                    $carousel.trigger('destroy.owl.carousel');
                    console.log('Destroyed existing Owl Carousel instance');
                } catch (e) {
    ...
    // Also initialize after window load to ensure all images are loaded
...
<!-- Override styles to move thumbnails to the left -->
<style>
/* This will override the previous styles to place thumbnails on the left */
.transparent-dots {
    left: 2.6rem !important; /* Ensure it's on the left side */
}

/* Ensure the gallery layout is correct */
.sidebar-wrapper {
    display: flex !important;
    flex-direction: row !important;
}

/* Order thumbnail container first */
.prod-thumbnail {
    order: 1 !important;
}

/* Order main image container second */
.product-slider-container {
    order: 2 !important;
}
</style>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(function() {
        ...
        if (!thumbnailContainer || !sliderContainer) return;
        
        // Make sure the thumbnail container comes before the slider container
        if (sliderContainer.compareDocumentPosition(thumbnailContainer) & Node.DOCUMENT_POSITION_FOLLOWING) {
            // If thumbnails appear after the slider, reorder them
            sidebarWrapper.insertBefore(thumbnailContainer, sliderContainer);
        }
    }, 500);
});
</script>
...
<!-- Fixed Product Gallery Styles -->
<style>
/* Improved Product Gallery with Vertical Layout for Desktop & Horizontal for Mobile */
.product-single-gallery {
    position: relative;
    margin-bottom: 30px;
}
...
@media (max-width: 991px) {
    .sidebar-wrapper {
        flex-direction: row !important; /* Keep the row layout for mobile */
    }
    
    .prod-thumbnail {
        flex-direction: column !important; /* Keep vertical thumbnails */
        width: 60px !important; /* Narrower thumbnails on mobile */
        margin-right: 10px !important;
        order: 1 !important; /* Keep thumbnails on the left */
        max-height: 320px !important; /* Limit height on mobile */
    ...
    .product-slider-container {
        flex: 1 !important;
        width: calc(100% - 70px) !important; /* Account for thumbnail width + margin */
        order: 2 !important; /* Keep main image on the right */
    }
...
/* Ensure the gallery is visible on mobile */
@media (max-width: 767px) {
    .prod-thumbnail {
        display: flex !important;
        width: 50px !important; /* Even narrower on small mobile */
    ...
    .product-slider-container {
        width: calc(100% - 58px) !important; /* Account for smaller thumbnails */
    }
    ...
    /* Make sure sidebar wrapper remains as a row */
    .sidebar-wrapper {
        display: flex !important;
        flex-direction: row !important;
    }
...
/* Additional styles to ensure proper behavior at all screen sizes */
@media (max-width: 480px) {
    .prod-thumbnail {
        width: 40px !important;
...
/* Force column layout on VERY small screens if needed */
@media (max-width: 360px) {
    .sidebar-wrapper {
        flex-direction: row !important; /* Still keep row layout */
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initProductGallery() {
        ...
        console.log('Initializing product gallery for both desktop and mobile');
        ...
        // Force the thumbnail container to be visible (important for mobile)
        if (thumbnailContainer) {
    ...
    // Additional initialization specifically for mobile
    function ensureMobileGalleryVisible() {
        if (window.innerWidth <= 991) {
            ...
            if (thumbnailContainer) {
                ...
                // For mobile: ensure thumbnails are after the main image
                if (sidebarWrapper && sidebarWrapper.children.length >= 2) {
                    // Move thumbnails after main slider if needed
                    const mainSlider = document.querySelector('.product-slider-container');
                    if (mainSlider && sidebarWrapper.children[0] === thumbnailContainer) {
                        sidebarWrapper.appendChild(thumbnailContainer);
                    }
                }
            }
        }
    ...
    ensureMobileGalleryVisible();
    window.addEventListener('resize', ensureMobileGalleryVisible);
});
</script>

<!-- Mobile Touch Support for Gallery -->
<script>
// Add swipe detection for mobile
document.addEventListener('DOMContentLoaded', function() {
    const mainImageContainer = document.querySelector('.product-slider-container');
