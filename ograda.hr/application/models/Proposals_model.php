...
class Proposals_model extends App_Model
{
    ...
    public function add($data)
    {
        ...
        $rate = (int)(isset($data['rate']) ? $data['rate'] : 0);
        ...
        if(isset($data['proposal'])){
            $cartProposal = 1;
            $frontdata['selected_height'] = $data['selected_height'];
            $frontdata['selected_material'] = $data['selected_material'];
            $frontdata['selected_color'] = $data['selected_color'];
            $frontdata['subject'] = $data['subject'];
            $data['proposal_to'] = $data['name'];
            
            $data['subject'] = 'Ponuda -' . $data['subject'];
            unset($data['proposal']);
            unset($data['selected_height']);
            unset($data['selected_material']);
            unset($data['selected_color']);
            unset($data['name']);
            //message add somewhere
            unset($data['message']);
        
            $data['total'] = $qty * $rate;
            $data['subtotal'] = $qty * $rate;
            $data['currency'] = 2;
            $data['date'] = date('Y-m-d');
            $data['open_till'] = date('Y-m-d', strtotime($data['date'] . '+7 days'));
        ...
        if($cartProposal){
            ...
            $desc = $data['subject'] . '<br>';
            if(!empty($frontdata['selected_height'])) {
                $desc = $desc . 'Height: ' . $frontdata['selected_height'] . '<br>';
            }
            if(!empty($frontdata['selected_material'])) {
                $desc = $desc . 'Material: ' . $frontdata['selected_material'] . '<br>';
            }
            if(!empty($frontdata['selected_color'])) {
                $desc = $desc . 'Color: ' . $frontdata['selected_color'] . '<br>';
            }

            $itemable['rel_id'] =  $insert_id;
            $itemable['rel_type'] =  'proposal';
            $itemable['description'] =  $data['subject'];
            $itemable['long_description'] =  $desc;
            $itemable['qty'] =  $qty;
            $itemable['rate'] =  $rate;
            $itemable['unit'] =  'm1';
            $itemable['height'] =  $frontdata['selected_height'];
        ...
        if ($insert_id) {
            ...
            if($cartProposal) {
                $proposal = $this->get($insert_id, [], true);
            } else {
